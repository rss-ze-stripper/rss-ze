...
function HpBarFrame(Frame){if(Segments==0)Segments = Frame;}
function ChangeHp(){if(BossHealth<=0){HpBar();Segments--;}else if(Segments<=0){BossKill();ticking=false;}}
function HpBar(){for(local i = Segments; i >= 0; i--){EntFireByHandle(self, "RunScriptCode", " ResetHealth() ", 0.00, null, null);return;}}
function Start()
{
     ticking = true;
	 Tick();
...
function StartShuff(number, two)
{
	 ...
	 else if(attack == 6)
	 {
	     if(attack == 6&&Yellow_att == 0)
		 {
			 ...
			 EntFire("fixyellow1", "Enable", "", 10.00, null);
			 EntFire("fixyellow2", "Enable", "", 10.00, null);
			 EntFire("fixyellow3", "Enable", "", 10.00, null);
			 EntFire("fixyellow4", "Enable", "", 10.00, null);
			 EntFire("fixyellow1", "Disable", "", 25.00, null);
			 EntFire("fixyellow2", "Disable", "", 25.00, null);
			 EntFire("fixyellow3", "Disable", "", 25.00, null);
			 EntFire("fixyellow4", "Disable", "", 25.00, null);
		     Yellow_att++;
			 Reset_att--;
		 }
